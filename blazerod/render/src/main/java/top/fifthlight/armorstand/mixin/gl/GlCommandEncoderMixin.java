...
import top.fifthlight.armorstand.render.gl.BufferClearer;
...
public abstract class GlCommandEncoderMixin implements CommandEncoderExt {
    ...
    private void checkRenderPassBuffers(RenderPassImpl pass, boolean checkIndexBuffer) {
        ...
        if (armorStandVertexBuffer != null) {
            if (armorStandVertexBuffer.getClosed()) {
    ...
    @Override
    public void armorStand$clearBuffer(GpuBufferSlice slice, ClearType clearType) {
        ...
        if (bufferManager.armorStand$isGlClearBufferObjectEnabled()) {
            bufferManager.armorStand$clearBufferData(buffer.id, slice.offset(), slice.length(), clearType);
        } else {
    ...
    @Override
    public void armorStand$copyBuffer(GpuBufferSlice target, GpuBufferSlice source) {
        ...
        var bufferManager = (BufferManagerExtInternal) this.backend.getBufferManager();
        bufferManager.armorStand$copyBuffer(targetBuffer.id, sourceBuffer.id, target.offset(), source.offset(), target.length());
    }
}
